...
\\
\\
Metristen avaruuksien peruskäsitteitä, kuten joukkojen välinen etäisyys ja normin yhteys metriikkaan, esitetään kolmannessa luvussa. Samassa luvussa esitetään homeomorfismin, upotuksen ja isometrian välisiä suhteita. Kuratowskin upotuslauseen todistuksessa hyödynnetyt rajoitettua metriikkaa koskevat lemmat \ref{rajmetri} ja \ref{rajmetri2} todistetaan. %3.11 ja 3.12
\\
\\
%Kuratowskin upotuslauseessa merkittävässä asemassa oleva konveksi verho esitellään omassa luvussaan
%Neljännessä luvussa käsitellään konveksin verhon ominaisuuksia aloittaen konveksiuden määrittelystä. Konveksi verho on keskeisimpiä käsitteitä Kuratowskin upotuslauseessa.
Kuratowskin upotuslauseen mukaan jokainen metrinen avaruus voidaan upottaa osaksi normiavaruutta niin, että upotuksen kuvajoukko on suljettu konveksissa verhossaan. Neljännessä luvussa käsitellään konveksin verhon ominaisuuksia aloittaen konveksiuden määrittelystä. Erityisesti konveksin verhon yhteys lineaarikombinaatioihin esitetään ja todistetaan lemmassa \ref{konveksilineaari}. %4.6
\\
\\
Tutkielman viimeisessä luvussa todistetaan Kuratowskin upotuslause. Lauseen todistamiseen tarvitaan esitietoja vektoriavaruuksien ja metrisistä avaruuksien ominaisuuksista. Luvussa todetaan, että upotuksen avulla jokainen metrinen avaruus voidaan kuvata jonkin normiavaruuden osajoukoksi, joka on suljettu konveksissa verhossaan.
%Todistuksessa hyödynnetään edellisten lukujen asioita.
...
Jos $a\neq 0$, niin $f=a^{-1}af$ ja edellisen yhtälön %$\ref{N2todistus1}$ 
mukaan $||f||\leq|a^{-1}|||af||$
...
%Olkoon $(X,d)$ metrinen avaruus. Tällöin metriikka $d$ on ekvivalentti lemmassa \ref{rajmetri} määritellyn metriikan $d'$ kanssa.
\begin{proof} %Olkoon $x\in X$. 
%Tällöin $B_d(x,r)=\{y\in X\colon d(x,y)< r\}$
%$B_{d'}(x,r')=\{y\in X\colon d'(x,y)< r'\}$
%Olkoon $(X,d)$ metrinen avaruus ja $d'(x,y)=\dfrac{d(x,y)}{1+d(x,y)}$ metriikka joukossa $X$. Tällöin
%Olkoon $X$ joukko ja $d$ ja $d'$ metriikkoja joukossa $X$. Tällöin
%$$d'\colon X\times X \rightarrow \R,\quad d'(x,y)=\dfrac{d(x,y)}{1+d(x,y)} \, , \qquad x,y\in X$$
%
%Olkoon $a,b\in X$. Tällöin $ r=d(a,b)$ ja $r'=d'(a,b)=\dfrac{d(a,b)}{1+d(a,b)}=\dfrac{r}{1+r} $. 
%Nyt $r=r'\cdot (1+r)=r'+r'r $ ja edelleen $r'=r-r'r=r(1-r') $ ja edelleen $ r=\dfrac{r'}{1-r'} $.
%\begin{equation*}
%\begin{split}
%B_d(x,r)=&\{y\in X\colon d(x,y)< r\}\\
%B_{d'}(x,r')=&\{y\in X\colon d'(x,y)< r'\}
%\end{split}
%\end{equation*}
...
%Merkit $<$ ja $>$ voidaan korvata $\leq$ ja $\geq$ merkeillä.
Vastaavasti $a\leq b$ jos, ja vain jos $\dfrac{a}{a+1}\leq \dfrac{b}{b+1}$.
%Vastaavasti jos $a\leq b$ niin $\dfrac{a}{a+1}\leq \dfrac{b}{b+1}$ ja jos $\dfrac{a}{a+1}\leq \dfrac{b}{b+1}$ niin $a\leq b$.
\end{proof}
\end{huom}
%\begin{lem}
%Olkoon $(X,d)$ metrinen avaruus. 
...
%\end{split}
%\end{equation*}
%kun $e+\lambda_{n+1} =1$. Valitsemalla $ed_i= \lambda_{i}$ kaikilla $i\in {0,1,\dots,n}$ saadaan 
%$$\lambda_0 x_0+\lambda_1 x_1+\cdots+\lambda_{n} x_{n}+ \lambda_{n+1} x_{n+1}
%\in C(A).$$ 
Siis $C(A)$ sisältää kaikki muotoa% lineaarikombinaatiot, jotka ovat muotoa
\begin{equation*}
\lambda_0 x_0+\lambda_1 x_1+\cdots+\lambda_{n} x_{n},
\quad\text{missä } x_i\in A, \lambda_i\geq0\text{, ja }
%\lambda_0+\lambda_1+\cdots+ \lambda_{n} =1
...
%Osoitetaan seuraavaksi, että $C(A)$ sisältää ainoastaan tätä muotoa olevia lineaarikombinaatiota. 
...
\end{split}
\end{equation*}
%sillä $(1-t)\lambda_0 +\cdots+(1-t)\lambda_{n} +t\lambda_0' +\cdots+t\lambda_{m}' =
% (1-t)(\lambda_0 +\cdots+\lambda_{n} )+t(\lambda_0'  +\cdots+\lambda_{m}' )$
...
%Siis joukko $A\subset B
%
%Olkoon
%$$B= \left\lbrace\sum_{i=0}^{n}{\lambda_i x_i}\colon x_i\in A, \lambda_i\in \R, \lambda_i \geq 0 \text{ ja } \sum_{i=0}^{n}{\lambda_i }=1 \right\rbrace. $$
%Aiemman perusteella $ B\subset C(A)$. Osoitetaan, että joukko $C(A)\subset B$, jolloin $B=C(A)$.
%Joukko $B$ on konveksi (edellinen) ja $A\subset B$, jolloin korollaarin \ref{konv-pienin} nojalla $C(A)\subset B$.
\end{proof}
